...
AFountain::AFountain()
{
	...
	Light->SetLightColor(FLinearColor(255.f, 0.f, 0.f));
	RotateSpeed = 30.f;
	Movement->RotationRate = FRotator(0.f, RotateSpeed, 0.0f);
}
...
{
	Super::BeginPlay();
	//UE_LOG(AreanaBattle, Warning, TEXT("Actor Name : %s, ID : %d, Location X : %.3f"), *GetName(), ID, GetActorLocation().X);
	ABLOG_S(Warning);
